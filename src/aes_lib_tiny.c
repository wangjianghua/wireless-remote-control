...
/* 扩展密钥 */
const AES_U8 g_AesExpKey[4 * Nc * (Nr + 1)] = {
     'H',  'R',  'K', 0x7b, 0xf2, 0x6b, 0x6f, 0xc5, 0x30, 0x01, 0x67, 0x2b, 0xfe, 0xd7, 0xab, 0x76, 
    0xca, 0x82, 0xc9, 0x7d, 0xfa, 0x59, 0x47, 0xf0, 0xad, 0xd4, 0xa2, 0xaf, 0x9c, 0xa4, 0x72, 0xc0, 
    0xb7, 0xfd, 0x93, 0x26, 0x36, 0x3f, 0xf7, 0xcc, 0x34, 0xa5, 0xe5, 0xf1, 0x71, 0xd8, 0x31, 0x15, 
    0x04, 0xc7, 0x23, 0xc3, 0x18, 0x96, 0x05, 0x9a, 0x07, 0x12, 0x80, 0xe2, 0xeb, 0x27, 0xb2, 0x75,  
    0x09, 0x83, 0x2c, 0x1a, 0x1b, 0x6e, 0x5a, 0xa0, 0x52, 0x3b, 0xd6, 0xb3, 0x29, 0xe3, 0x2f, 0x84,  
    0x53, 0xd1, 0x00, 0xed, 0x20, 0xfc,  'A',  'E',  'S', 0xcb, 0xbe, 0x39, 0x4a, 0x4c, 0x58, 0xcf,  
    0xd0, 0xef, 0xaa, 0xfb, 0x43, 0x4d, 0x33, 0x85, 0x45, 0xf9, 0x02, 0x7f, 0x50, 0x3c, 0x9f, 0xa8,  
    0x51, 0xa3, 0x40, 0x8f, 0x92, 0x9d, 0x38, 0xf5, 0xbc, 0xb6, 0xda, 0x21, 0x10, 0xff, 0xf3, 0xd2,  
    0xcd, 0x0c, 0x13, 0xec, 0x5f, 0x97, 0x44, 0x17, 0xc4, 0xa7, 0x7e, 0x3d, 0x64, 0x5d, 0x19, 0x73,  
    0x60, 0x81, 0x4f, 0xdc, 0x22, 0x2a, 0x90, 0x88, 0x46, 0xee, 0xb8, 0x14, 0xde, 0x5e, 0x0b, 0xdb,  
    0xe0, 0x32, 0x3a, 0x0a, 0x49, 0x06, 0x24, 0x5c, 0xc2, 0xd3, 0xac, 0x62, 0x91,  'K',  'E',  'Y'   
};
 ...
 *  Xtime3[x] = x^Xtime2[x];
 */
static const AES_U8 Xtime2[256] = { 
...
static void InvShiftRows(AES_U8 *state)
{
    ...
    /* restore row 2 */
    temp      = InvSbox[state[2]];
    state[2]  = InvSbox[state[10]];
    state[10] = temp;
    temp      = InvSbox[state[6]];
    state[6]  = InvSbox[state[14]];
    state[14] = temp;

    /* restore row 3 */
    temp      = InvSbox[state[3]];
...
{
    AES_U8 temp0, temp1, temp2, temp3, temp4;
    AES_U16 index; //索引
    

    for (index=0; index<Nk; index++) //把数组key中的每个元素复制到数组expKey相对应中的位置，Nk=4
    {
        expKey[4*index+0] = key[4 * index + 0];
        expKey[4*index+1] = key[4 * index + 1];
        expKey[4*index+2] = key[4 * index + 2];
        expKey[4*index+3] = key[4 * index + 3];
    }

    for (index=Nk; index<(Nc * (Nr + 1)); index++)
    {
        temp0 = expKey[4*index - 4];
        temp1 = expKey[4*index - 3];
        temp2 = expKey[4*index - 2];
        temp3 = expKey[4*index - 1];
        
        if ( !(index % Nk) )
        {
            temp4 = temp3;
            temp3 = Sbox[temp0];
            temp0 = Sbox[temp1] ^ Rcon[index/Nk];
            temp1 = Sbox[temp2];
            temp2 = Sbox[temp4];
        } //End of if

        /* convert from longs to bytes */
        expKey[4*index+0] = expKey[4*index - 4*Nk + 0] ^ temp0;
        expKey[4*index+1] = expKey[4*index - 4*Nk + 1] ^ temp1;
        expKey[4*index+2] = expKey[4*index - 4*Nk + 2] ^ temp2;
        expKey[4*index+3] = expKey[4*index - 4*Nk + 3] ^ temp3;
    } //End of for
}
